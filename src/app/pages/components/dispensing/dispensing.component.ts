...
export class DispensinComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.screeningForm = this.fb.group({
      ...
      // New Q3 Toggle Control
      frame_selected: [null, Validators.required],
  ...
  // ---------- Frame Selected Toggle Handler ----------
  selectFrameSelected(value: boolean) {
    this.screeningForm.get('frame_selected')?.setValue(value);
    this.screeningForm.get('frame_selected')?.markAsTouched();
    ...
    if (!value) {
      // If "No", clear out frame fields and remove all requirements
      fd.setValue('');
      fn.setValue('');
      fbm.setValue('');
      fd.clearValidators();
      ...
      // If "Yes", re-apply requirements based on the current lenses_type selection
      const currentLens = this.f['lenses_type'].value;
      this.applyFrameFieldRequirements(currentLens);
    }
  ...
  private loadDispensing(reference_number: string) {
    this.apiService.getDispensing(reference_number).subscribe(res => {
      if (res && !res.error && res.body) {
        ...
        this.screeningForm.enable(); // keep your read-only UX for existing record
      }
    },err=>{
...
selectFrameSelected(value: boolean) {
  ...
  private mapApiToForm(data: any) {
    return {
      frame_selected: data?.frame_selected ?? (data?.frame_distance || data?.frame_near || data?.frame_bifocal ? true : null),
      lenses_type: data?.lenses_type ?? '',
      pd_distance: this.toNumOrNull(data?.pd_distance),
      pd_near: this.toNumOrNull(data?.pd_near),
      frame_distance: data?.frame_distance ?? '',
  ...
  private loadDispensing(reference_number: string) {
    this.apiService.getDispensing(reference_number).subscribe(res => {
      if (res && !res.error && res.body) {
        ...
        // Apply conditional requireds based on loaded frame selection and lens type
        const frameSelected = this.screeningForm.get('frame_selected')?.value;
        if (frameSelected === true && data?.lenses_type) {
          this.applyFrameFieldRequirements(data.lenses_type);
        }
  ...
  // ---------- Lens type → frame field requirements ----------
  onLensesChange(lensType: string) {
  // Apply frame field requirements whenever the lens type changes
  if (this.screeningForm.get('frame_selected')?.value === true) {
    this.applyFrameFieldRequirements(lensType);
  }

  // Handle fitting heights for Step 2 based on lens type
  const fr = this.f['fitting_height_re'];
  const fl = this.f['fitting_height_le'];

  if (lensType !== 'Bifocals' && lensType !== 'multifocals') {
    fr.setValue('NO');
    fl.setValue('NO');
    fr.clearValidators();
    fl.clearValidators();
  } else {
    // Re-enable validation/clearing if they switch back to bifocals/multifocals
    fr.setValue('');
    fl.setValue('');
    // add back any validators if fitting height is required for bifocals, or leave optional
  }
  fr.updateValueAndValidity();
  fl.updateValueAndValidity();
}

  private applyFrameFieldRequirements(lenses: string) {
    const frameSelected = this.f['frame_selected'].value;
    const fd = this.f['frame_distance'];
    const fn = this.f['frame_near'];
    const fbm = this.f['frame_bifocal'];
    ...
    // Only apply validation if frames are actually selected ("Yes")
...
// ---------- Wizard nav ----------
  nextStep() {
    ...
    if (this.currentStep === 1) {
      // Base controls that must always be valid
      const controlsToCheck: AbstractControl[] = [
        this.f['frame_selected'], 
        this.f['lenses_type'], 
        this.f['pd_distance'], 
        this.f['pd_near']
      ];

      // Only include specific frame fields in the validation check if frames are selected ("Yes")
      const frameSelected = this.f['frame_selected'].value;
      if (frameSelected === true) {
        const lenses = this.f['lenses_type'].value;
        if (lenses === 'Single Vision Distance only') {
          controlsToCheck.push(this.f['frame_distance']);
        } else if (lenses === 'Single vision near only') {
          controlsToCheck.push(this.f['frame_near']);
        } else if (lenses === 'both single vision distance and single vision near - 2pairs') {
          controlsToCheck.push(this.f['frame_distance'], this.f['frame_near']);
        } else if (lenses === 'Bifocals' || lenses === 'multifocals') {
          controlsToCheck.push(this.f['frame_bifocal']);
        }
      }
  ...
  submitForm() {
    ...
    const payload = {
      reference_number: this.reference_number,
      frame_selected: this.f['frame_selected'].value,
      lenses_type: this.f['lenses_type'].value,
      pd_distance: this.toNumOrNull(this.f['pd_distance'].value),
      pd_near: this.toNumOrNull(this.f['pd_near'].value),
      frame_distance: this.f['frame_distance'].value,
